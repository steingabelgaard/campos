...
class PortalGroupReg(CustomerPortal):
    def _group_reg_check_access(self, ckr_id, access_token=None):
        ...
        try:
            ...
            if not access_token or not consteq(
                ckr_sudo.access_token, access_token
            ):
                raise
        return ckr_sudo
    ...
    def portal_group_reg_add_1(self, **post):
        ...
        values = {
            'group': {'country_id': country_da_id},
            'contact': {},
            'error': {},
            'countries': countries,
        }
        ...
        if request.httprequest.method == 'POST':
            ...
            if email and '@' in email:
                if partner_sudo.search([('email', '=ilike', email)]):
                    return request.render(
                        "campos_group_reg.group_add_email_exist",
                        {'email': email},
                    )
            ...
            if not error:
                ...
                values = {
                    'group': group,
                    'contact': contact,
                    'scout_orgs': request.env['campos.scout.org']
                    .sudo()
                    .search(scout_org_domain),
                    'error': error,
                    'countries': countries,
                    'group_reg': group_reg,
                }
            ...
            if group_reg_id:
                ...
                if group_reg:
                    ...
                    if post.get('group_scout_org_id', False):
                        group_reg.scout_org_id = post.get(
                            'group_scout_org_id', False
                        )

                    if post.get('same_as_contact', False) == 'reuse':
                        group_reg.treasurer_partner_id = (
                            group_reg.contact_partner_id
                        )
                    else:
                        treasurer = {}
                        tres_vals = {}
                            ...
                            for f in [
                                ...
                                if not post.get('treasurer_' + f):
                                    request.website.add_status_message(
                                        _('Missing required field: %s') % f,
                                        type_='danger',
                                    )
                                    values['treasurer'] = tres_vals
                                    values['group_reg'] = group_reg
                                    return request.render(
                                        "campos_group_reg.group_add_3", values
                                    )
                    ...
                    if not group_reg.treasurer_partner_id:
                        values['treasurer'] = {
                            'country_id': group_reg.country_id.id
                        }
                        return request.render(
                            "campos_group_reg.group_add_3", values
                        )
                    ...
                    return request.render(
                        "campos_group_reg.thank_you_login", {}
                    )

                    group_reg.write(
                        {'prereg_ids': group_reg._default_prereg_ids()}
                    )
                    values['group_reg'] = group_reg

                    periods = (
                        request.env['campos.camp.period'].sudo().search([])
                    )
                    agegroups = (
                        request.env['campos.age.group'].sudo().search([])
                    )
                    cell = {}
                    for ag in agegroups:
                        cell[ag.id] = {}
                    ...
                    return request.render(
                        "campos_group_reg.group_pre_reg", values
                    )

        return request.render("campos_group_reg.group_add_1", values)

    @http.route(
        ['/group-reg/pre-reg'], type='http', auth="public", website=True
    )
        ...
        if request.httprequest.method == 'POST':
            ...
            if group_reg_id:
                ...
                if group_reg:
                    for c in group_reg.prereg_ids:
                        c.value = post.get(
                            'cell_%s_%s' % (c.age_group_id.id, c.period_id.id),
                            False,
                        )
    ...
    def portal_my_pre_reg(self, **post):
        ...
        if partner.parent_id.scoutgroup:
                ...
                if group_reg_id:
                    ...
                    if group_reg:
                        for c in group_reg.prereg_ids:
                            c.value = post.get(
                                'cell_%s_%s'
                                % (c.age_group_id.id, c.period_id.id),
                                False,
                            )
                if request.website:
                    request.website.add_status_message(
                        _('Pre registration figures has been updated.')
                    )
            ...
            return request.render(
                "campos_group_reg.group_pre_reg_edit", values
            )
        return request.redirect('/my')
    ...
    @http.route(
        ['/my/ckr/submit/<int:ckr_id>/<access_token>/'],
        type='http',
        auth="public",
        methods=['POST'],
        website=True,
